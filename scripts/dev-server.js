...
const server = http.createServer((req, res) => {
  ...
  if (routes[pathname]) {
    ...
    return filePath ? serveFile(res, filePath) : res.writeHead(403).end('Forbidden');
  }

  // Serve assets under /assets from data/assets
  if (pathname.startsWith('/assets/')) {
    const assetPath = pathname.replace('/assets/', 'assets/');
    const filePath = safeJoin(ROOT, assetPath);
    return filePath ? serveFile(res, filePath) : res.writeHead(404).end('Not Found');
  }

  // Direct file access (e.g., /style.css, /app.js)
...
server.listen(PORT, () => {
  ...
  console.log(`[dev-server] Set DEVICE_HOST env var to change device address (default: ${DEVICE_HOST})`);
});
