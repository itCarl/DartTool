    ...
    populatePlayers(gameData) {
        ...
        gameData.players.forEach((player) => {
            ...
            const lastThrowsHTML = lastThrowValues.map((points, idx) => {
                const borderClass = idx < 2 ? 'border-r' : '';
                return `<div class="${borderClass} flex justify-center items-center">${points}</div>`;
            }).join('');

            const currentPlayerStyle = isCurrentPlayer ? 'box-shadow: -6px 0 0 0 #dc2626;' : '';

            item.innerHTML = `
                <div class="grid grid-cols-3 bg-black/70 rounded-r w-full text-center mb-2" style="${currentPlayerStyle}">
                    <div class="flex flex-col border-r">
                        <div class="text-2xl font-bold p-4">${remainingPoints}</div>
                        <div class="border-t py-1">
                            ${player.name}
                        </div>
                    </div>
                    <div class="grid grid-cols-3 border-r">
                        ${lastThrowsHTML}
                        <div class="col-span-3 border-t text-2xl flex justify-center items-center">
                            ${lastRoundTotal}
                        </div>
                    </div>
                    <div class="grid grid-cols-2">
                        <div class="border-r flex flex-col justify-center items-center">
                            <span class="text-lg font-semibold">${throwCount}</span>
                        </div>
                        <div class="flex flex-col justify-center items-center">
                            <span class="text-lg font-semibold">${totalScored}</span>
                        </div>
                        <div class="col-span-2 border-t text-2xl flex justify-center items-center">
